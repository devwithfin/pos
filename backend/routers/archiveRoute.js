...
router.get('/archived', async (req, res) => {
  try {
    const tables = ['categories', 'customers', 'suppliers', 'products', 'roles'];
    const typeMap = {
      categories: 'category',
      customers: 'customer',
      suppliers: 'supplier',
    ...
    for (const table of tables) {
      const [rows] = await db.query(`SELECT id, name, deleted_at FROM ${table} WHERE deleted_at IS NOT NULL`);
...
router.put('/restore/:type/:id', async (req, res) => {
  ...
  const validTypes = ['categories', 'products', 'suppliers', 'customers', 'roles'];
